...
def get_clean_merged_df(use_cached=True):
    """
    Loads Zillow Challengs data, removes and fills in missing data,
    caches this calculation, then returns a dataframe only including
    residential houses.

    use_chached: attempts to load cached data
    """
    merged_fname = 'data/merged.hdf5'
    properties_root = 'data/properties_2016'
    train_root = 'data/train_2016'
    if os.access(merged_fname, os.F_OK) and use_cached:
        merged = pd.read_hdf(merged_fname, 'merged')
    else:
        if os.access(properties_root + '.hdf5', os.F_OK) \
           and os.access(train_root + '.hdf5', os.F_OK) \
           and use_cached:
            traits_df = pd.read_hdf(properties_root + '.hdf5', 'traits_df')
            score_df = pd.read_hdf(train_root + '.hdf5', 'score_df')
        else:
            traits_df = pd.read_csv(properties_root + '.csv')
            score_df = pd.read_csv(train_root + '.csv')
            traits_df.to_hdf(properties_root + '.hdf5', 'traits_df', mode='w')
            score_df.to_hdf(train_root + '.hdf5', 'score_df', mode='w')
        ...
        merged.to_hdf(merged_fname, 'merged', mode='w')
    return merged
